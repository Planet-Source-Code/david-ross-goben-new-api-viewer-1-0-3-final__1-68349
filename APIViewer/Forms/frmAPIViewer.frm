...
Begin VB.Form frmAPIViewer 
   ...
   Begin VB.ListBox LstItems 
      ...
      EndProperty
      Height          =   1620
      ItemData        =   "frmAPIViewer.frx":3898
      Left            =   240
      List            =   "frmAPIViewer.frx":389A
      Sorted          =   -1  'True
      TabIndex        =   5
      Top             =   1560
      Width           =   5535
   ...
   Begin VB.ComboBox cboAPIType 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "frmAPIViewer.frx":389C
      Left            =   240
      List            =   "frmAPIViewer.frx":38AC
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "T&ype the first few letters of the word you are looking for:"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
'
  ...
  #If ISADDIN = 0 Then
    ...
    Me.imgAlert(0).Top = Me.cmdDepends.Top
    Me.imgAlert(1).Top = Me.cmdDepends.Top
  #End If
...
'
  ...
  Me.imgAlert(0).Visible = False  'dependency alert image
...
'*******************************************************************************
' Subroutine Name   : imgAlert_Click
' Purpose           : User can turn off alert by clicking it
'*******************************************************************************
Private Sub imgAlert_Click(Index As Integer)
...
'
  ...
  If CBool(InStr(1, T, " As Any")) Then
    SetMsg "THIS DECLARATION CONTAINS AT LEAST ONE ""AS ANY"" CLAUSE"
  Else
    SetMsg vbNullString
  End If
End Sub
...
Private Function OpenFile(Path As String) As Boolean
  ...
  If Fso.FileExists(Path) Then
    ...
    DoEvents
    SetMsg "Loading " & Path & "..."                    'report loading
''''-------------------
''''
...
Private Sub tmrAlert_Timer()
  Me.imgAlert(0).Visible = Not Me.imgAlert(0).Visible
End Sub
...
'*******************************************************************************
Private Sub SetMsg(Text As String)
  Me.tmrMsg.Enabled = False
  ...
  End With
  Me.tmrMsg.Enabled = CBool(Len(Text))
End Sub
